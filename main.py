...
def get_teams(folder):
    ...
    for file in files:
        ...
        players = [
            x.strip() + "_" + suffix for x in open(folder + "/" + file, "r").readlines()
        ]
...
def generate_fixtures(teams, pivot=None, save_folder=None):
    ...
    if save_folder:
        ...
        qualifiers_df = pd.DataFrame(
            [
                {
                    "rounds": "qualifiers_" + str(i),
                    "player1": value.split("_$vs$_")[0],
                    "player2": value.split("_$vs$_")[1],
                }
                for i, value in enumerate(qualifiers)
            ]
        ).reset_index()
        qualifiers_df.to_csv(save_folder + "/qualifiers.csv")

        knockouts_df = pd.DataFrame(
            [
                {
                    "rounds": "round_of_" + str(pivot * 4) + "_" + str(i),
                    "player1": value.split("_$vs$_")[0],
                    "player2": value.split("_$vs$_")[1],
                }
                for i, value in enumerate(knockouts)
            ]
        ).reset_index()
        qualifiers_df.to_csv(save_folder + "/qualifiers.csv")
...
def divide_fixtures(player_set):
    ...
    if match_length == 1:
        data = [
            {
                "rounds": "round_of_2_0",
                "winner_of_1": "round_of_4_0",
                "winner_of_2": "round_of_4_1",
            }
        ]
        ...
        fixtures = [
            {
                "rounds": "round_of_" + str(len(player_set)) + "_" + str(i),
                "winner_of_1": value[0],
                "winner_of_2": value[1],
            }
...
def generate_static_fixtures(pivot, save_folder):
    init_round = pivot * 4
    player_set = [
        "round_of_" + str(init_round) + "_" + str(i) for i in range(2 * pivot)
    ]
    further_set = []
    ...
    for _ in range(rounds):
        ...
        further_set.append(
            {
                "fixtures": divide_fixtures(player_set),
                "round": "round_of_" + str(len(player_set)),
            }
        )
        further_set[-1]["fixtures"].to_csv(
            save_folder + "/" + further_set[-1]["round"] + ".csv"
        )
        init_round = init_round // 2
        player_set = [
            "round_of_" + str(init_round) + "_" + str(i)
            for i in range(len(player_set) // 2)
        ]
    return further_set
...
if __name__ == "__main__":
    ...
    starting_round_multiple = 3

    out_folder = f"games/badminton/{category}"

    inp_folder = f"teams/badminton/{category}"
    teams = get_teams(inp_folder)
    print(generate_fixtures(teams, starting_round_multiple, save_folder=out_folder))
    generate_static_fixtures(starting_round_multiple, save_folder=out_folder)
